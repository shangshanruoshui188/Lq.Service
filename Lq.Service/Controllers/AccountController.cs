...
namespace Lq.Service.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> Register(RegisterBindingModel model)
        {
            ...
            var fullType = "Lq.Service.Models." + model.Type;
            var userObj = CreateInstance(fullType);
        ...
        {
            object obj = CreateInstance(className, args);
            return (T)obj;
        }

        /// <summary>
        /// 按照命名空间和类名初始化实例，命名空间和类名忽略大小写
        /// </summary>
        /// <param name="ns">命名空间，大小写不敏感</param>
        /// <param name="className">类名，大小写不敏感</param>
        /// <param name="args"></param>
        /// <returns></returns>
        public static object CreateInstance(string ns, string className, params object[] args)
        {
            string fullName = string.Format("{0}.{1}", ns, className);
            return CreateInstance(fullName, args);
        }

        /// <summary>
        /// 以大小写不敏感的方式根据类全名初始化实例
        /// </summary>
        ...
        public static object CreateInstance(string className, params object[] args)
        {
            try
            {
                Type type = GetLocalType(className);
        ...
        /// 忽略大小写的方式获取本地指定全名的类型
        /// </summary>
        /// <param name="fullName">类全名</param>
        /// <returns></returns>
        public static Type GetLocalType(string fullName)
        {
            return Assembly.GetExecutingAssembly().GetTypes().ToList().Find(t => t.FullName.Equals(fullName, StringComparison.OrdinalIgnoreCase));
        }
